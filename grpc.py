    ...
    def _assist(self, recorder, play, deadline):
        with Leds() as leds:
            ...
            for response in self._assistant.Assist(self._requests(recorder), deadline):
                ...
                # ---------------------------
                # carwash.py
                no_carwash = ["rain", "snow", "showers", "hail"]
                yes_carwash = ["sunny", "cloudy"]
                interior.on()

                # to store temperature data
                temperatures = re.findall(r'\b\d+\b', supplemental_display_text)
                # prints numbers onto a list, ie. ['52, '49']
#                print(str(temperatures))

                if any([x in supplemental_display_text for x in no_carwash]):
                    logger.info('RAIN DETECTED!!!!')
                    logger.info('Window up and LED blinking blue')
                    blueLED.on()
                    tuned_servo.min()
                ...
                elif any([x in supplemental_display_text for x in yes_carwash]):
                    logger.info('NO RAIN DETECTED!!!!')
                    logger.info('Window down and LED breathing yellow')
                    yellowLED.on()
                    tuned_servo.max()
